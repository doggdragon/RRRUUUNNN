...
static void oled_scl_write(bsp_io_level_t level)
{
    R_IOPORT_PinWrite(&g_ioport_ctrl, OLED_PIN_SCL, level);
}

static void oled_sda_write(bsp_io_level_t level)
{
    R_IOPORT_PinWrite(&g_ioport_ctrl, OLED_PIN_SDA, level);
}

static void oled_i2c_start(void)
{
    oled_sda_write(BSP_IO_LEVEL_HIGH);
    oled_scl_write(BSP_IO_LEVEL_HIGH);
    oled_i2c_delay();
    oled_sda_write(BSP_IO_LEVEL_LOW);
    oled_i2c_delay();
    oled_scl_write(BSP_IO_LEVEL_LOW);
}
...
static void oled_i2c_send_byte(uint8_t dat)
{
    ...
    for (i = 0; i < 8; i++)
    {
        oled_sda_write((dat & 0x80U) ? BSP_IO_LEVEL_HIGH : BSP_IO_LEVEL_LOW);
        oled_i2c_delay();
        oled_scl_write(BSP_IO_LEVEL_HIGH);
        oled_i2c_delay();
        oled_scl_write(BSP_IO_LEVEL_LOW);
    ...
    /* ACK clock: keep SDA released (high) and ignore returned level. */
    oled_sda_write(BSP_IO_LEVEL_HIGH);
    oled_i2c_delay();
    oled_scl_write(BSP_IO_LEVEL_HIGH);
    oled_i2c_delay();
...
void OLED_WR_Byte(u8 dat,u8 mode)
{
    ...
    oled_i2c_send_byte(mode ? 0x40U : 0x00U);
    oled_i2c_send_byte(dat);
    oled_i2c_stop();
}
...
void OLED_Refresh(void)
{
    ...
    for(i=0;i<oled_y;i++)
    {
        ...
        oled_i2c_start();
        oled_i2c_send_byte(OLED_I2C_ADDR_WRITE);
        oled_i2c_send_byte(0x40U);
        for(n=0;n<oled_x;n++)
        {
            oled_i2c_send_byte(OLED_GRAM[n][i]);
        }
        oled_i2c_stop();
    }
...
void OLED_RefreshPage(u8 page)
{
    ...
    OLED_WR_Byte((u8)(0xb0 + page), OLED_CMD);
    OLED_WR_Byte(0x00, OLED_CMD);
    OLED_WR_Byte(0x10, OLED_CMD);
    ...
    for (n = 0; n < oled_x; n++)
    {
        oled_i2c_send_byte(OLED_GRAM[n][page]);
    }
    oled_i2c_stop();
}
//清屏函数
